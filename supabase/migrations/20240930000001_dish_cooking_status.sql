-- Migration to add dish cooking status tracking for admin orders dashboard
-- This allows tracking individual dish cooking status within orders

-- Add cooking status tracking to order_items
ALTER TABLE public.order_items 
ADD COLUMN cooking_status TEXT DEFAULT 'pending' CHECK (cooking_status IN ('pending', 'cooking', 'cooked'));

-- Add cooking timestamps
ALTER TABLE public.order_items 
ADD COLUMN cooking_started_at TIMESTAMP WITH TIME ZONE,
ADD COLUMN cooking_completed_at TIMESTAMP WITH TIME ZONE;

-- Create dish cooking batches table for bulk cooking operations
...
-- Function to update order item cooking status when batch is completed
...
-- Trigger to update order items when batch status changes
...
-- Function to get weekly dish summary for cooking dashboard
...
-- Function to get orders with dish cooking status for admin dashboard
...
RETURNS TABLE (
    ...
    user_name TEXT,
    user_email TEXT,
    delivery_group TEXT,  -- Changed from INTEGER to TEXT to match the enum type
    order_status TEXT,
    delivery_date DATE,
    total_items INTEGER,
    ...
    protein_option TEXT
    -- Removed cooking status columns as they don't exist in the original schema
) AS $$
BEGIN
    RETURN QUERY
    SELECT 
        ...
        o.delivery_group::TEXT,  -- Cast enum to TEXT
        ...
        oi.protein_option
        -- Removed cooking status columns from SELECT
    FROM public.orders o
    JOIN public.users u ON o.user_id = u.id
    JOIN public.order_items oi ON o.id = oi.order_id
    JOIN public.dishes d ON oi.dish_id = d.id
    WHERE (status_filter IS NULL OR o.status::TEXT = status_filter)  -- Cast enum to TEXT for comparison
...
-- Function to start cooking a dish batch
...
-- Function to complete cooking a dish batch
...
CREATE INDEX idx_order_items_dish_cooking ON public.order_items(dish_id, cooking_status);
CREATE INDEX idx_dish_cooking_batches_date ON public.dish_cooking_batches(batch_date);
CREATE INDEX idx_dish_cooking_batches_status ON public.dish_cooking_batches(status);
